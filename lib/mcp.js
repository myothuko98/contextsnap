...
export async function runPipeline({ dirs, ignore = [], cwd = process.cwd() }) {
  ...
  return compileMarkdown(scannedFiles, baseDirAbs, { workspaces: await detectWorkspaces(cwd) });
}
...
export async function startMcpServer() {
  ...
  const dirsParam = z.array(z.string()).optional().describe(
    'Directories to scan relative to the project root. Defaults to common util folders (src/utils, lib, src, …).'
  );
  ...
  const rootParam = z.string().optional().describe(
    'Absolute path to the project root. REQUIRED when the MCP host does not start this server in the project directory (Claude Desktop starts servers in / or the home directory). Defaults to the server process cwd.'
  );
  ...
  server.tool(
    ...
    { dirs: dirsParam, ignore: ignoreParam, root: rootParam },
    async ({ dirs, ignore, root }) => {
  ...
  server.tool(
    ...
    {
      path: z.string().describe('File path relative to the project root (or absolute).'),
      root: rootParam,
    },
    async ({ path: file, root }) => {
      const markdown = await fileContext({ file, cwd: root ?? process.cwd() });
      return { content: [{ type: 'text', text: markdown }] };
    }
  );
